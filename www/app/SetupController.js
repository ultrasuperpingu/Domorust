define(['app'], function (app) {
	app.controller('SetupController', ['$scope', '$rootScope', '$window', '$location', '$http', '$interval', 'md5', function ($scope, $rootScope, $window, $location, $http, $interval, md5) {
		...
		$scope.AllowNewHardware = function (minutes) {
			$.ajax({
				url: "domorust-api/devices/allow_new?timeout=" + minutes,
				async: false,
				dataType: 'json',
		...
		$scope.TestNotification = function (subsystem) {
			...
			$.ajax({
				url: "domorust-api?type=command&param=testnotification&subsystem=" + subsystem + "&" + extraparams,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (data.status != "OK") {
		...
		$scope.ShowSettings = function () {
			...
			$.ajax({
				url: "domorust-api?type=command&param=getdynamicpricedevices",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (typeof data.MeterDividerEnergy != 'undefined') {
						$("#rfxmetertable #MeterDividerEnergy").val(data.MeterDividerEnergy);
					}
					...
					if (typeof data.MeterDividerGas != 'undefined') {
						$("#rfxmetertable #MeterDividerGas").val(data.MeterDividerGas);
					}
					if (typeof data.CostGas != 'undefined') {
						$("#rfxmetertable #CostGas").val(data.CostGas);
					}
					if (typeof data.MeterDividerWater != 'undefined') {
						$("#rfxmetertable #MeterDividerWater").val(data.MeterDividerWater);
					}
					if (typeof data.CostWater != 'undefined') {
						$("#rfxmetertable #CostWater").val(data.CostWater);
					}
					if (typeof data.RandomTimerFrame != 'undefined') {
						$("#randomtable #RandomTimerFrame").val(data.RandomTimerFrame);
					}
					if (typeof data.SensorTimeout != 'undefined') {
						$("#timeouttable #SensorTimeout").val(data.SensorTimeout);
					}
					if (typeof data.BatteryLowNotification != 'undefined') {
						$("#batterytable #BatteryLowNotification").val(data.BatteryLowNotification);
					}
		...
		$scope.StoreSettings = function (formname) {
			...
			$http.put('domorust-api/settings', new FormData(document.querySelector("#settings")), {
				...
				HideNotify();
				ShowNotify($.t("Settings saved"), 2000);
		...
		$scope.CleanupShortLog = function () {
			bootbox.confirm($.t("Are you sure to delete the Log?\n\nThis action can not be undone!"), function (result) {
				if (result == true) {
					$.ajax({
						url: "domorust-api?type=command&param=clearshortlog&idx=" + $.devIdx,
						async: false,
						dataType: 'json',
						success: function (data) {
							$window.location = '/#Dashboard';
