define(['app', 'livesocket'], function (app) {
	app.controller('FloorplanController', function ($scope, $rootScope, $location, $window, $http, $interval, $timeout, $compile, permissions, livesocket) {
		...
		RefreshFPDevices = function () {
			...
			/*livesocket.getJson("domorust-api/devices?filter=all&used=true&order=Name&lastupdate=" + $.LastUpdateTime, function (data) {
			...
			$.ajax({
				url: "domorust-api/devices?filter=all&used=true&order=Name&lastupdate=" + $.LastUpdateTime,
				async: false,
				dataType: 'json',
		...
		$scope.ShowFPDevices = function (floorIdx) {
			...
			$http({
				url: "domorust-api/devices?filter=all&used=true&order=Name&floor=" + $scope.floorPlans[floorIdx].idx
		...
		$scope.ShowRooms = function (floorIdx) {
			$http({
				//url: "domorust-api?type=command&param=getfloorplanplans&idx=" + $scope.floorPlans[floorIdx].idx
				//url: "domorust-api/floorplans/" + $scope.floorPlans[floorIdx].idx + "/plans"
				url: "domorust-api/plans?FloorplanID=" + $scope.floorPlans[floorIdx].idx
			}).then(function successCallback(response) {
				var data = response.data;
				if (typeof data.result != 'undefined') {
					$.each(data.result, function (i, item) {
		...
		ShowFloorplans = function () {
			...
			$http({
				url: "domorust-api/floorplans", async: false
			}).then(function successCallback(response) {
				var data = response.data;
				if (typeof data.result != 'undefined') {
