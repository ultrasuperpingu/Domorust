define(['app', 'livesocket'], function (app) {
	app.controller('TemperatureController', function ($scope, $rootScope, $location, $http, $interval, $window, $route, $routeParams, deviceApi, permissions, livesocket) {
		...
		RefreshTemps = function () {
			...
			/*livesocket.getJson("domorust-api/devices?filter=temp&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId, function (data) {
			...
			$.ajax({
				url: "domorust-api/devices?filter=temp&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
		...
		ShowTemps = function () {
			...
			$.ajax({
				url: "domorust-api/devices?filter=temp&used=true&order=[Order]&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
