	...
	app.controller('UserVariablesController', ['$scope', '$rootScope', '$location', '$http', '$interval', function ($scope, $rootScope, $location, $http, $interval) {
		...
		DeleteVariable = function (idx) {
			bootbox.confirm($.t("Are you sure you want to remove this variable?"), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/user_variables/" + idx,
						async: false,
						dataType: 'json',
		...
		AddUpdateVariable = function (type, idx) {
			...
			if ((type == "a") && (jQuery.inArray(uservariablename, $scope.varNames) != -1)) {
				...
				var method="";
				if (type == "a") {
					url = "domorust-api/user_variables?vname=" + uservariablename + "&vtype=" + uservariabletype + "&vvalue=" + uservariablevalue;
					method="POST";
				}
				else if (type == "u") {
					url = "domorust-api/user_variables/" + idx + "?vname=" + uservariablename + "&vtype=" + uservariabletype + "&vvalue=" + uservariablevalue;
					method="PUT";
				}
