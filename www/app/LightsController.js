define(['app', 'livesocket'], function (app) {
	app.controller('LightsController', function ($scope, $rootScope, $location, $http, $interval, $route, $routeParams, deviceApi, permissions, livesocket) {
		...
		RefreshHardwareComboArray = function () {
			$.ComboHardware = [];
			$.ajax({
				url: "domorust-api?type=command&param=getmanualhardware",
				async: false,
				dataType: 'json',
				success: function (data) {
		...
		RefreshLights = function () {
			...
			/*livesocket.getJson("domorust-api/devices?filter=light&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId, function (data) {
			...
			$.ajax({
				url: "domorust-api/devices?filter=light&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.ServerTime != 'undefined') {
		...
		ShowLights = function () {
			...
			$.ajax({
				url: "domorust-api/devices?filter=light&used=true&order=[Order]&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
				success: function (data) {
					...
					if (window.myglobals.ismobileint == false) {
						...
						$element.find(".span4").droppable({
							drop: function () {
								...
								$.ajax({
									url: "domorust-api?type=command&param=switchdeviceorder&idx1=" + myid + "&idx2=" + $.devIdx + "&roomid=" + roomid,
									async: false,
									dataType: 'json',
									success: function (data) {
										ShowLights();
		...
		TestManualLight = function () {
			...
			$.ajax({
				url: "domorust-api?type=command&param=testswitch" + mParams,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.status != 'undefined') {
		...
		function init() {
			...
			//Get SwitchTypes
			$.ajax({
				url: "domorust-api/infos/switchtypes",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
				...
				buttons: {
					"Add Device": function () {
						...
						if (bValid) {
							$(this).dialog("close");
							$.ajax({
								url: "domorust-api?type=command&param=setused&idx=" + $.devIdx + '&name=' + encodeURIComponent($("#dialog-addlightdevice #devicename").val()) + '&switchtype=' + $("#dialog-addlightdevice #comboswitchtype").val() + '&used=true&maindeviceidx=' + MainDeviceIdx,
								async: false,
								dataType: 'json',
								success: function (data) {
									ShowLights();
			...
			dialog_addmanuallightdevice_buttons[$.t("Add Device")] = function () {
				...
				$.pDialog = $(this);
				$.ajax({
					url: "domorust-api?type=command&param=addswitch" + mParams,
					async: false,
					dataType: 'json',
					success: function (data) {
						if (typeof data.status != 'undefined') {
