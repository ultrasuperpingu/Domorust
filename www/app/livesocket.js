	...
	*/
	module.service('livesocket', function ($websocket, $http, $rootScope, $q, $location, notifyBrowser) {
		...
		function getJson(url, callback_fn) {
			...
			var use_http = !(url.substr(0, 13) == "domorust-api?");
			...
			if (use_http) {
				...
				return sendRequest(url.substr(13)).then(callback_fn);
			}
		}
	...
	$.ajax = function (settings) {
		if (settings.url.substr(0, 9) == "domorust-api?" && settings.url.match(/param=getdevices/)) {
