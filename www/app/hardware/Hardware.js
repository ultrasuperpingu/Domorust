	...
	app.controller('HardwareController', function ($scope, $rootScope, $timeout, $http) {
	  ...
	  return true;
		...
		DeleteHardware = function (idx) {
			bootbox.confirm($.t("Are you sure to delete this Hardware?\n\nThis action can not be undone...\nAll Devices attached will be removed!"), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/hardwares/" + idx,
						async: false,
						dataType: 'json',
		...
		ActionDomoticzHW = function(mode, hardwaretype, logLevel, timeout, enabled, idx) {
			...
			$.ajax({
				url: "domorust-api/hardwares"+idxTxt+"?htype=" + hardwaretype +
				"&loglevel=" + logLevel +
		...
		UpdateHardware = function (idx, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6) {
			...
			if (!$.isNumeric(hardwaretype)) {
				...
				if (bIsOK) {
					$.ajax({
						method:"PUT",
						url: "domorust-api/hardwares/"+idx+"?htype=94" +
						"&loglevel=" + logLevel +
						"&idx=" + idx +
			...
			/*if (text.indexOf("1-Wire") >= 0) {
				...
				$.ajax({
					method:"PUT",
					url: "domorust-api/hardwares/"+idx+"?htype=" + hardwaretype +
					"&loglevel=" + logLevel +
				...
				)
			 {
				...
				$.ajax({
					method:"PUT",
					url: "domorust-api/hardwares/"+idx+"?htype=" + hardwaretype + "&name=" + encodeURIComponent(name) +
					"&loglevel=" + logLevel +
		...
		AddHardware = function () {
			...
			if (!$.isNumeric(hardwaretype)) {
				...
				if (bIsOK) {
					$.ajax({
						method:"POST",
						url: "domorust-api/hardwares?htype=94" +
						"&loglevel=" + logLevel +
						"&name=" + encodeURIComponent(name) +
						"&username=" + encodeURIComponent(($(selector + " #Username").length == 0) ? "" : $(selector + " #Username").val()) +
						"&password=" + encodeURIComponent(($(selector + " #Password").length == 0) ? "" : $(selector + " #Password").val()) +
			...
			/*if (text.indexOf("1-Wire") >= 0) {
				...
				$.ajax({
					method:"POST",
					url: "domorust-api/hardwares?htype=" + hardwaretype + "&extra=" + encodeURIComponent(owfspath) +
					"&loglevel=" + logLevel +
				...
				$.ajax({
					method:"POST",
					url: "domorust-api/hardwares?htype=" + hardwaretype + "&name=" + encodeURIComponent(name) +
					"&loglevel=" + logLevel +
					"&enabled=" + bEnabled + "&datatimeout=" + datatimeout	+
					"&extra=" + encodeURIComponent($("#hardwarecontent #hardwareparamsrtl433 #rtl433cmdline").val()),
					async: false,
				...
				$.ajax({
					method:"POST",
					url: "domorust-api/hardwares?htype=" + hardwaretype +
					"&loglevel=" + logLevel +
					"&address=" + address +
					"&port=" + port +
					"&name=" + encodeURIComponent(name) +
				...
				$.ajax({
					method:"POST",
					url: "domorust-api/hardwares?htype=" + hardwaretype + "&name=" + encodeURIComponent(name) + "&enabled=" + bEnabled + "&datatimeout=" + datatimeout +
					"&loglevel=" + logLevel +
					"&address=" + encodeURIComponent(i2caddress) +
					"&port=" + encodeURIComponent(i2cpath) +
					"&Mode1=" + encodeURIComponent(i2cinvert),
				...
				var location = $("#hardwarecontent #divopenweathermap #location").val();
				if (location == "") {
					ShowNotify($.t('Please enter an Location (or 0 to use Domorust own location)!'), 2500, true);
					return;
				}
				var adddayforecast = $("#hardwarecontent #divopenweathermap #adddayforecast").prop("checked") ? 1 : 0;
				var addhourforecast = $("#hardwarecontent #divopenweathermap #addhourforecast").prop("checked") ? 1 : 0;
		...
		SetCCUSBType = function () {
			$http({
				url: "domorust-api?type=command&param=setccmetertype&idx=" + $('#hardwarecontent #ccusbtype #idx').val() + "&CCBaudrate=" + $('#hardwarecontent #ccusbtype #CCBaudrate').val()
		...
		RefreshLMSNodeTable = function () {
			...
			$.ajax({
				url: "domorust-api?type=command&param=lmsgetnodes&idx=" + $.devIdx,
				async: false,
				dataType: 'json',
				success: function (data) {
		...
		SetLMSSettings = function () {
			...
			if (Mode1 < 1)
				Mode1 = 30;
			$.ajax({
				url: "domorust-api?type=command&param=lmssetmode" +
				"&idx=" + $.devIdx +
				"&mode1=" + Mode1,
				async: false,
				dataType: 'json',
		...
		DeleteUnusedLMSDevices = function () {
			bootbox.confirm($.t("Are you sure to delete all unused devices?"), function (result) {
				if (result == true) {
					$.ajax({
						url: "domorust-api?type=command&param=lmsdeleteunuseddevices" +
							"&idx=" + $.devIdx,
						async: false,
						dataType: 'json',
						success: function (data) {
		...
		SendOTGWCommand = function () {
			...
			$.ajax({
				url: "domorust-api?type=command&param=sendopenthermcommand" +
				"&idx=" + idx +
				"&cmnd=" + cmnd,
				async: false,
				dataType: 'json',
		...
		EditOpenTherm = function (idx, name, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6) {
			...
			$.ajax({
				url: "domorust-api/devices?filter=temp&used=true&order=Name",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
...
*/
		...
		BindEvohome = function (idx, name, devtype) {
			...
			if (devtype == "Relay")
				...
				ShowNotify($.t('Binding Domorust zone temperature sensor to controller...'), 2500);
			else
				ShowNotify($.t('Binding Domorust outdoor temperature device to evohome controller...'), 2500);
			...
			setTimeout(function () {
				...
				$.ajax({
					url: "domorust-api?type=command&param=bindevohome&idx=" + $.devIdx + "&devtype=" + devtype,
					async: false,
					dataType: 'json',
					success: function (data) {
			...
			$("#dialog-createevohomesensor").dialog({
				...
				buttons: {
					"OK": function () {
						...
						$.ajax({
							url: "domorust-api?type=command&param=createevohomesensor&idx=" + $.devIdx +
			...
			$("#dialog-createrflinkdevice").dialog({
				...
				buttons: {
					"OK": function () {
						...
						$.ajax({
							url: "domorust-api?type=command&param=createrflinkdevice&idx=" + $.devIdx +
			...
			$("#dialog-createsensor").dialog({
				...
				buttons: {
					"OK": function () {
						...
						$.ajax({
							url: "domorust-api?type=command&param=createdevice&idx=" + $.devIdx +
			...
			$("#dialog-addyeelight").dialog({
				...
				buttons: {
					"OK": function () {
						...
						$(this).dialog("close");
						$.ajax({
							url: "domorust-api?type=command&param=addyeelight&idx=" + $.devIdx +
			...
			$("#dialog-addArilux").dialog({
				...
				buttons: {
					"OK": function () {
						...
						$(this).dialog("close");
						$.ajax({
							url: "domorust-api?type=command&param=addArilux&idx=" + $.devIdx +
							"&name=" + encodeURIComponent(SensorName) +
							"&ipaddress=" + encodeURIComponent(IPAddress) +
							"&stype=" + SensorType,
							async: false,
		...
		TellstickApplySettings = function () {
			...
			if (repeatInterval > 2000)
				repeatInterval = 2000;
			$.ajax({
				url: "domorust-api?type=command&param=tellstickApplySettings" +
				"&idx=" + $.idx +
				"&repeats=" + repeats +
		...
		RegisterPhilipsHue = function () {
			...
			var username = $("#hardwarecontent #hardwareparamsphilipshue #username").val();
			$.ajax({
				url: "domorust-api?type=command&param=registerhue" +
				"&ipaddress=" + address +
				"&port=" + port +
				"&username=" + encodeURIComponent(username),
				async: false,
		...
		function init() {
			...
			$.ajax({
				url: "domorust-api/infos/serial_devices",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
			...
			$.ajax({
				url: "domorust-api/infos/hardwaretypes",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
						...
						$.each(data.result, function (i, item) {
							...
							//}
							//else
							{  // For Python Plugins build the input fields
