...
//			return;
    ...
    var activeForced = (arguments.length === 1 ? true: false);
		...
		$.each(setPoints, function(ndx, tsettings){
			...
			var urlcmd =  "domorust-api?type=command&param=" + defaults.commandAdd +"&idx=" + defaults.devid +
	...
	clearPrevSetPoints = function(callbackFunc, param){
		...
		$.each(prevTimers, function(ndx, timerid){
			timers.push("domorust-api?type=command&param=delete"+ (defaults.temperatureModes ? "setpoint" : "") + "timer&idx=" + timerid);
		});
