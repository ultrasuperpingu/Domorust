...
*/
	...
	function createTimersApiFactory(timerType, $q, domoticzApi) {
		...
		function getTimers(deviceIdx) {
			return domoticzApi.get( 'devices/'+deviceIdx+'/timers', {idx: deviceIdx})
				.then(function (data) {
		...
		function addTimer(deviceIdx, config) {
			return domoticzApi.add('devices/'+deviceIdx+'/timers', Object.assign({}, config, {idx: deviceIdx}));
		}

		function updateTimer(timerIdx, config) {
			return domoticzApi.update(timerType+"s/"+timerIdx, Object.assign({}, config, {idx: timerIdx}));
		}

		function clearTimers(deviceIdx) {
			return domoticzApi.del('devices/'+deviceIdx+'/timers', {idx: deviceIdx});
		}

		function deleteTimer(timerIdx) {
			return domoticzApi.del(timerType+"s/"+timerIdx, {idx: timerIdx});
		}
	}
});