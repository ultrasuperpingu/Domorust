define(['app'], function (app) {
	app.controller('FloorplanEditController', ['$scope', '$rootScope', '$location', '$http', '$interval', '$compile', 'permissions', function ($scope, $rootScope, $location, $http, $interval, $compile, permissions) {
		...
		ChangeFloorplanOrder = function (order, floorplanid) {
			...
			$.ajax({
				url: "domorust-api?type=command&param=changefloorplanorder&idx=" + floorplanid + "&way=" + order,
				async: false,
				dataType: 'json',
		...
		DeleteFloorplan = function (idx) {
			bootbox.confirm($.t("Are you sure you want to delete this Floorplan?"), function (result) {
				if (result == true) {
					$.ajax({
						url: "domorust-api?type=command&param=deletefloorplan&idx=" + idx,
						async: false,
						dataType: 'json',
						error: function () {
							HideNotify();
		...
		UpdateFloorplan = function (idx) {
			...
			$.ajax({
				url: "domorust-api?type=command&param=updatefloorplan&idx=" + idx + "&name=" + csettings.name + "&scalefactor=" + csettings.scalefactor,
				async: false,
				dataType: 'json',
				success: function (data) {
					RefreshFloorPlanTable();
		...
		RefreshUnusedDevicesComboArray = function () {
			...
			$.ajax({
				url: "domorust-api?type=command&param=getunusedfloorplanplans&unique=false",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
		...
		RefreshPlanTable = function (idx) {
			...
			$.ajax({
				//url: "domorust-api/floorplans/" + idx + "/plans",
				url: "domorust-api/plans?FloorPlanID=" + idx,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
		...
		AddUnusedPlan = function () {
			...
			$.ajax({
				method:"POST",
				url: "domorust-api/floorplans/" + PlanIdx + "/plans?" + "planidx=" + PlanIdx,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (data.status == 'OK') {
		...
		UpdateFloorplanPlan = function (planidx, clear) {
			...
			$.ajax({
				method:"PUT",
				url: "domorust-api/plans/" + planidx + 
				"?area=" + PlanArea,
				async: false,
				dataType: 'json',
				success: function (data) {
			...
			for (var i = 0; i < deviceParent.childNodes.length; i++) {
				if (deviceParent.childNodes[i].getAttribute('onfloorplan') != 'false') {
					$.ajax({
						method:"PUT",
						url: "domorust-api/plans/" + planidx +"/devices/" + deviceParent.childNodes[i].getAttribute('idx') +
		...
		RefreshDevices = function () {
			if ((typeof $("#floorplangroup") != 'undefined') &&
				...
				if ($("#floorplangroup")[0].getAttribute("planidx") != "") {
					Device.useSVGtags = true;
					$http({
						url: "domorust-api/devices?filter=all&used=true&order=Name&plan=" + $("#floorplangroup")[0].getAttribute("planidx") + "&lastupdate=" + window.myglobals.LastUpdate
					}).then(function successCallback(response) {
