define(['app'], function (app) {
	app.controller('UpdateController', ['$scope', '$rootScope', '$location', '$http', '$interval', '$window', function ($scope, $rootScope, $location, $http, $interval, $window) {
		...
		$scope.CheckUpdateReader = function() {
			...
			$http({
				method : "GET",
				url : "domorust-api/infos/version",
				timeout: 5000
			}).then(
				function mySuccess(response) {
					var data = response.data;
					if (data.status == "OK") {
						$scope.newVersion = data.result[0];
		...
		$scope.StartUpdate = function() {
			...
			$http({
				method : "GET",
				url : "domorust-api?type=command&param=update_application",
			}).then(
				function mySuccess(response) {
					var data = response.data;
					if (data.status == "OK") {
		...
		$scope.CheckForUpdate = function() {
			...
			$http({
				method : "GET",
				url : "domorust-api?type=command&param=checkforupdate&forced=true"
			}).then(
				function mySuccess(response) {
					var data = response.data;
