define(['app'], function (app) {
	app.factory('domoticzEventsApi', function ($http, $httpParamSerializer, $q, domoticzApi) {
		...
		function fetchCurrentStates() {
			return domoticzApi.get('events_scripts/devices_current_status', {
		...
		function listEvents() {
			return domoticzApi.get('events_scripts', {
				evparam: 'list'
			}).then(function (data) {
				return {
					events: data.result || [],
		...
		function loadEvent(eventId) {
			return domoticzApi.get('events_scripts/' + eventId, {
				evparam: 'load',
				event: eventId
			}).then(function (data) {
		...
		function deleteEvent(eventId) {
			return domoticzApi.del('events_scripts/'+eventId, {
				evparam: 'delete',
				event: eventId
			});
		}
