	...
	app.controller('ScenesController', function ($scope, $rootScope, $location, $http, $interval, permissions, livesocket) {
		...
		RemoveCode = function (idx, code) {
			...
			bootbox.confirm($.t("Are you sure to delete this Device?\n\nThis action can not be undone..."), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/scenes/"+ SceneIdx +"/activations?sceneidx=" + SceneIdx + "&idx=" + idx + "&code=" + code,
						async: false,
						dataType: 'json',
		...
		AddCode = function () {
			...
			setTimeout(function () {
				...
				$.ajax({
					url: "domorust-api?type=command&param=learnsw",
					async: false,
					dataType: 'json',
					success: function (data) {
				...
				setTimeout(function () {
					if (bHaveFoundDevice == true) {
						$.ajax({
							method:"POST",
							url: "domorust-api/scenes/" + SceneIdx + "/activations/sceneidx=" + SceneIdx + "&idx=" + deviceidx + "&cmnd=" + Cmd,
							async: false,
							dataType: 'json',
							success: function (data) {
								RefreshActivators();
		...
		ClearCodes = function () {
			...
			bootbox.confirm($.t("Are you sure to delete ALL Devices?\n\nThis action can not be undone!"), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/scenes/" + SceneIdx + "/activations?sceneidx=" + SceneIdx,
						async: false,
						dataType: 'json',
						success: function (data) {
							RefreshActivators();
		...
		DeleteScene = function () {
			bootbox.confirm($.t("Are you sure to remove this Scene?"), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/scenes/" + SceneIdx,
						async: false,
						dataType: 'json',
						success: function (data) {
							ShowScenes();
		...
		SaveScene = function () {
			...
			if (bValid) {
				...
				$.ajax({
					method:"PUT",
					url: "domorust-api/scene/" + SceneIdx +
					"?scenetype=" + SceneType +
		...
		AddDevice = function () {
			...
			$.ajax({
				method:"POST",
				url: "domorust-api/scenes/"+ SceneIdx + "devices?idx=" + SceneIdx + "&isscene=" + $.isScene + "&devidx=" + DeviceIdx + "&command=" + Command + "&level=" + level + "&color=" + colorJSON + "&ondelay=" + ondelay + "&offdelay=" + offdelay,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (data.status == 'OK') {
		...
		ClearDevices = function () {
			...
			bootbox.confirm($.t("Are you sure to delete ALL Devices?\n\nThis action can not be undone!"), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/scenes/" + SceneIdx + "/devices",
						async: false,
						dataType: 'json',
						success: function (data) {
							RefreshDeviceTable(SceneIdx);
		...
		MakeFavorite = function (id, isfavorite) {
			...
			$.ajax({
				method:"PUT",
				url: "domorust-api/scenes/"+id+"/make_favorite?favorite=" + isfavorite,
				async: false,
				dataType: 'json',
				success: function (data) {
					ShowScenes();
		...
		ChangeDeviceOrder = function (order, devid) {
			...
			$.ajax({
				url: "domorust-api/scenes/changescenedeviceorder?idx=" + devid + "&way=" + order,
				async: false,
				dataType: 'json',
				success: function (data) {
		...
		SetColValue = function (idx, color, brightness) {
			...
			$.ajax({
				url: "domorust-api?type=command&param=setcolbrightnessvalue&idx=" + idx + "&color=" + color + "&brightness=" + brightness,
				async: false,
				dataType: 'json'
			});
		}
		...
		RefreshActivators = function () {
			...
			$.ajax({
				url: "domorust-api/scenes/scenes/"+ SceneIdx+"/activations",
				async: false,
				dataType: 'json',
				success: function (data) {
		...
		RefreshDeviceTable = function (idx) {
			...
			$.ajax({
				url: "domorust-api/scenes/"+ idx +"/devices?isscene=" + $.isScene,
				async: false,
				dataType: 'json',
				success: function (data) {
		...
		UpdateDevice = function (idx, devidx) {
			...
			$.ajax({
				url:"PUT",
				url: "domorust-api/scenes/" + idx + "/devices/"+ DeviceIdx + "?isscene=" + $.isScene + "&devidx=" + DeviceIdx + "&command=" + Command + "&level=" + level + "&color=" + colorJSON + "&ondelay=" + ondelay + "&offdelay=" + offdelay,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (data.status == 'OK') {
		...
		DeleteDevice = function (idx) {
			bootbox.confirm($.t("Are you sure to delete this Device?\n\nThis action can not be undone..."), function (result) {
				if (result == true) {
					$.ajax({
						method:"DELETE",
						url: "domorust-api/scenes/"+idx+"/devices/" + idx,
						async: false,
						dataType: 'json',
						success: function (data) {
							RefreshDeviceTable(SceneIdx);
		...
		RefreshLightSwitchesComboArray = function () {
			$.LightsAndSwitches = [];
			$.ajax({
				url: "domorust-api/devices/light_switches",
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
		...
		RefreshScenes = function () {
			/*livesocket.getJson("domorust-api?type=command&param=getscenes&lastupdate=" + $.LastUpdateTime, function (data) {
			...
			$.ajax({
				url: "domorust-api/scenes?lastupdate=" + $.LastUpdateTime,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.ServerTime != 'undefined') {
					...
					if (window.myglobals.ismobileint == false) {
						...
						$element.find(".span4").droppable({
							drop: function () {
								...
								$.ajax({
									url: "domorust-api?type=command&param=switchsceneorder&idx1=" + myid + "&idx2=" + SceneIdx,
									async: false,
									dataType: 'json',
									success: function (data) {
										ShowScenes();
				...
				buttons: [{
					...
					click: function () {
						...
						if (bValid) {
							...
							$.ajax({
								method:"POST",
								url: "domorust-api/scenes?name=" + SceneName + "&scenetype=" + SceneType,
								async: false,
								dataType: 'json',
								success: function (data) {
									if (typeof data.status != 'undefined') {
