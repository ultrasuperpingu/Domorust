define(['app', 'livesocket'], function (app) {
	app.controller('UtilityController', function ($scope, $rootScope, $location, $http, $interval, $route, $routeParams, deviceApi, domoticzApi, permissions, livesocket) {
		...
		LoadCustomIcons = function () {
			...
			//Get Custom icons
			$.ajax({
				url: "domorust-api?type=command&param=custom_light_icons",
				async: false,
				dataType: 'json',
		...
		RefreshUtilities = function () {
			...
			/*livesocket.getJson("domorust-api/devices?filter=utility&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId, function (data) {
			...
			$.ajax({
				url: "domorust-api/devices?filter=utility&used=true&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.ServerTime != 'undefined') {
		...
		ShowUtilities = function () {
			...
			$.ajax({
				url: "domorust-api/devices?filter=utility&used=true&order=[Order]&plan=" + roomPlanId,
				async: false,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
		...
		function populatemetertypes() {
			domoticzApi.get('infos/metertypes', {})
				.then(function (data) {
					if ( data.status === 'OK' ) {
		...
		function init() {
			...
			dialog_editutilitydevice_buttons[$.t("Update")] = function () {
				...
				if (bValid) {
					...
					$(this).dialog("close");
					$.ajax({
						url: "domorust-api?type=command&param=setused&idx=" + $.devIdx +
