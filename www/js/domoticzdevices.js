...
function Slider(event) {
    ...
    this.Click = function (event) {
        ...
        if (newLevel != curLevel) {
            ...
            $.ajax({
				method:"POST",
                url: "domorust-api/devices/" + idx  +"/command/set_level?" + "idx=" + idx + "&switchcmd=Set%20Level&level=" + newLevel,
                async: true,
                dataType: 'json'
            });
...
Device.MakeFavorite = function (id, isfavorite) {
    ...
    $.ajax({
        url: "domorust-api?type=command&param=makefavorite&idx=" + id + "&isfavorite=" + isfavorite,
        async: false,
        dataType: 'json',
