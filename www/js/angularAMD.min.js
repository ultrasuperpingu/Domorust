...
define(function(){'use strict';var bootstrapped=false,app_name,orig_app,alt_app,run_injector,config_injector,app_cached_providers={},orig_angular,alt_angular,onDemandLoader={},preBootstrapLoaderQueue=[],alternate_modules={},alternate_modules_tracker={},alternate_queue=[];function checkBootstrapped(){if(!bootstrapped){throw new Error('angularAMD not initialized.  Need to call angularAMD.bootstrap(app) first.');}}
    ...
    AngularAMD.prototype.route=function(config){var load_controller;if(config.hasOwnProperty('controllerUrl')){load_controller=config.controllerUrl;delete config.controllerUrl;if(typeof config.controller==='undefined'){config.controller=['$scope','__AAMDCtrl','$injector',function($scope,__AAMDCtrl,$injector){if(typeof __AAMDCtrl!=='undefined'){$injector.invoke(__AAMDCtrl,this,{'$scope':$scope});}}];}}else if(typeof config.controller==='string'){load_controller=config.controller;}
        if(load_controller){var resolve=config.resolve||{};resolve['__AAMDCtrl']=['$q','$rootScope',function($q,$rootScope){var defer=$q.defer();require([load_controller],function(ctrl){defer.resolve(ctrl);$rootScope.$apply();});return defer.promise;}];config.resolve=resolve;}
        return config;};AngularAMD.prototype.appname=function(){checkBootstrapped();return app_name;};AngularAMD.prototype.processQueue=function(){checkBootstrapped();if(typeof alt_angular==='undefined'){throw new Error('Alternate angular not set.  Make sure that `enable_ngload` option has been set when calling angularAMD.bootstrap');}
        function processRunBlock(block){run_injector.invoke(block);}
        for(var i=0;i<alternate_queue.length;i++){var item=alternate_queue[i],invokeQueue=item.module._invokeQueue,y;for(y=0;y<invokeQueue.length;y+=1){var q=invokeQueue[y],provider=q[0],method=q[1],args=q[2];if(app_cached_providers.hasOwnProperty(provider)){var cachedProvider;if(provider==='$injector'&&method==='invoke'){cachedProvider=config_injector;}else{cachedProvider=app_cached_providers[provider];}
            cachedProvider[method].apply(null,args);}else{if(window.console){window.console.error('"'+provider+'" not found!!!');}}}
            if(item.module._configBlocks){var configBlocks=item.module._configBlocks;for(y=0;y<configBlocks.length;y+=1){var cf=configBlocks[y],cf_method=cf[1],cf_args=cf[2];config_injector[cf_method].apply(null,cf_args);}}}
        while(alternate_queue.length){var item=alternate_queue.shift();if(item.module._runBlocks){angular.forEach(item.module._runBlocks,processRunBlock);}}
        alternate_modules={};};AngularAMD.prototype.getCachedProvider=function(provider_name){checkBootstrapped();var cachedProvider;switch(provider_name){case'__orig_angular':cachedProvider=orig_angular;break;case'__alt_angular':cachedProvider=alt_angular;break;case'__orig_app':cachedProvider=orig_app;break;case'__alt_app':cachedProvider=alt_app;break;default:cachedProvider=app_cached_providers[provider_name];}
        return cachedProvider;};AngularAMD.prototype.inject=function(){checkBootstrapped();return run_injector.invoke.apply(null,arguments);};AngularAMD.prototype.config=function(){checkBootstrapped();return config_injector.invoke.apply(null,arguments);};AngularAMD.prototype.reset=function(){if(typeof orig_angular==='undefined'){return;}
        window.angular=orig_angular;if(require.defined('angular')){require.undef('angular');define('angular',[],orig_angular);}
