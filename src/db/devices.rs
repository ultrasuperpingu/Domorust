...
use domorust_models::device_data::DeviceData;
use domorust_models::FromSqlTable;
use rusqlite::Connection;

use crate::domoticz::consts::{pTypeBARO, pTypeHUM, pTypeTEMP, pTypeTEMP_BARO, pTypeTEMP_HUM, DEVICE_SUBTYPES_DESC, DEVICE_TYPES_DESC, SWITCH_TYPES};
use crate::domoticz::{get_humidity_status, is_light_or_switch, is_light_switch, is_temp, is_utility, is_weather};

pub fn get_devices(params: HashMap<String,String>) -> Result<Vec<Device>, Box<dyn Error>> {
	let connection = Connection::open("domorust.db")?;
	let res=Device::get_items_from_table(&connection, &params);
	if let Ok(mut devices) = res {
		for d in &mut devices {
			...
			d.SubTypeName = DEVICE_SUBTYPES_DESC.get(&(d.Type as u8, d.SubType as u8)).unwrap_or(&"").to_string();
			...
			params_data.insert("DeviceID".to_string(), d.ID.to_string());
			d.Data2 = DeviceData::get_items_from_table(&connection, &params_data)?;
		}
...
pub fn get_device(id: usize, params: HashMap<String,String>) -> Result<Device, Box<dyn Error>> {
...
pub fn get_hardware_devices(hid: usize, params: HashMap<String,String>) -> Result<Vec<Device>, Box<dyn Error>> {
	let mut params = params.clone();
	params.insert("HardwareID".to_string(), hid.to_string());
	get_devices(params)
}
pub fn get_light_switches(params: HashMap<String,String>)  -> Result<Vec<Device>, Box<dyn Error>> {
